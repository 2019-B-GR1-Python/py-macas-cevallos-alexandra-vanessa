import scrapy 
# importar un paquete de scrapy
# las arañas son clases
def guardar_archivo(titulo, link, precio):
    import csv
    with open('books_to_scrape.csv','a', newline='') as csvfile:
...
class IntroSpider(scrapy.Spider):
    ...
    # nombre de la araña para ejecutar
    ...
    def parse(self, response):
        ...
        print(lista_links)
        precios = etiqueta_contenedora.css('div.product_price > p.price_color::text').extract()[0:]
        print(precios)
        lista_precios=[]
        for p in precios:
            p = p[1:]
